...
status_by_year <- read.csv("cleanded_date.csv")

# Now you can use kable to print it
#knitr::kable(status_by_year)
...
```{r}
#| label: tbl-severity
#| tbl-cap: Number of Establishment Have Five Different Severity Level in Different Year
#| echo: false

severity_by_year <- dinesafe_program_clean |>
  group_by(inspection_year, severity) |>
  summarise(count = n(), .groups = "keep") |>
  print(kable(severity_by_year))
...
```
By the figure@fig-severity above, it shows the distribution of different level. From it, it express that most of establishments in Toronto have no problems during inspection and the number of significant is large. This means there are certain number of establishments exist.
